...
public class ClienteChat extends JFrame implements ActionListener
{
	...
	public ClienteChat(Socket socket, String nombre)
	{
		...
		try
		{
			...
			String texto = "SERVIDOR: ¡Buenas " + nombre + "! Debes adivinar un número del 1 al 100";
	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource()==boton)
		{
			...
			int b; 
			b = Integer.parseInt(mensaje.getText());
			...
			try
			{
				...
				if(b == a) 
				{	
					fsalida.writeUTF("¡" + nombre + " ha acertado! El número secreto era " + b); 
					fsalida.writeUTF("Se ha deshabilitado el botón de Enviar, cierre la aplicación cuando quiera.");
					boton.setEnabled(false);
					// para cerrar la ventana al ganar -> repetir = false;
					
				}
				if(b < a) 
				{
					fsalida.writeUTF(nombre + " dice que el número es " + b + ", pero el número es MAYOR");
				}
				if(b > a) 
				{
					fsalida.writeUTF(nombre + " dice que el número es " + b + ", pero el número es MENOR");
				}
		...
		{
			String texto = "SERVIDOR: El usuario " + nombre + " se ha ido";
			try
			{
				fsalida.writeUTF(texto);
				fsalida.writeUTF("*");
